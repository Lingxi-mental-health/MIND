...
class FSDPSFTTrainer(object):
    ...
    def validation_step(self, batch: TensorDict, return_outputs=False):
        ...
        with torch.no_grad():
            ...
            if return_outputs:
                # Return logits for generating samples
                with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
                    output = self.fsdp_model(input_ids=batch['input_ids'],
                                           attention_mask=batch['attention_mask'],
                                           position_ids=batch['position_ids'],
    ...
    def generate_validation_samples_fast(self, num_samples=10):
        """Generate and display validation samples using inference mode (bypass FSDP)"""
        self.fsdp_model.eval()
        rank = self.device_mesh.get_rank()
        
        if rank == 0:
            print("\n" + "="*100)
            print(f"生成 {num_samples} 条验证样本 (推理模式-绕过FSDP):")
            print("="*100 + "\n")
        
        # Collect full model state on rank 0 for fast inference
        from torch.distributed.fsdp import FullStateDictConfig, StateDictType
        cfg = FullStateDictConfig(offload_to_cpu=False, rank0_only=True)
        
        with FSDP.state_dict_type(self.fsdp_model, StateDictType.FULL_STATE_DICT, cfg):
            if rank == 0:
                print("收集完整模型参数到rank 0...")
                state_dict = self.fsdp_model.state_dict()
                
                # Load into a non-FSDP model for fast generation
                from transformers import AutoModelForCausalLM, AutoConfig
                config = AutoConfig.from_pretrained(self.config.model.partial_pretrain, 
                                                   trust_remote_code=self.config.model.trust_remote_code)
                ...
                inference_model = AutoModelForCausalLM.from_pretrained(
                    ...
                    attn_implementation='flash_attention_2',
                    trust_remote_code=self.config.model.trust_remote_code,
                    device_map='cuda:0'  # Load on single GPU
                )
                
                # Load trained weights
                print("加载训练权重...")
                if self.config.model.get('lora_rank', 0) > 0:
                    # For LoRA model, load base model then LoRA weights
                ...
                with torch.no_grad():
                    for idx in range(num_samples):
                        # Get original prompt and response from dataset
                        prompt = self.val_dataset.prompts[idx]
                        ground_truth_response = self.val_dataset.responses[idx]
                        
                        # Apply chat template to get the input text
                        from ragen.utils import apply_chat_template
                        prompt_chat_str, _ = apply_chat_template(
                            self.tokenizer, 
                            prompt, 
                            "",  # empty response for generation
                            with_thinking=self.config.data.with_thinking,
                            add_generation_prompt=True, 
                            tokenize=False
                        )
                        
                        # Tokenize input
                        prompt_ids = self.tokenizer(prompt_chat_str, return_tensors='pt', add_special_tokens=False)
                        input_ids = prompt_ids['input_ids'].cuda()
                        attention_mask = prompt_ids['attention_mask'].cuda()
                        
                        # Generate with the inference model (much faster!)
                        with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
    ...
    def fit(self):
        ...
        if self.config.trainer.validate_before_training:
            ...
            # self.generate_validation_samples_fast(num_samples=1)
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            for data in self.train_dataloader:
                ...
                if global_step >= self.total_training_steps:
                    ...
                    # Generate validation samples at final step
                    # self.generate_validation_samples_fast(num_samples=1)
            ...
            # Generate validation samples at the end of each epoch
            # self.generate_validation_samples_fast(num_samples=1)
