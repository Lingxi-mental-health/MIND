...
def extract_diagnosis_label(row_dict: dict) -> str:
    """从数据行中提取诊断标签"""
...
class RLHFDataset(Dataset):
    ...
    def _read_files_and_tokenize(self):
        ...
        # 提取诊断标签用于均衡采样
        self._extract_diagnosis_labels()
    
    def _extract_diagnosis_labels(self):
        """提取诊断标签并计算采样权重"""
        labels = []
        for idx in range(len(self.dataframe)):
            row_dict = self.dataframe.iloc[idx].to_dict()
            label = extract_diagnosis_label(row_dict)
            labels.append(label)
        
        self.diagnosis_labels = labels
        
        # 统计类别分布
        from collections import Counter
        label_counts = Counter(labels)
        print(f"诊断类别分布: {dict(label_counts)}")
        ...
        print(f"类别采样权重: {class_weights}")
        
        # 为每个样本分配权重
        self.sample_weights = [class_weights[label] for label in labels]
    
    def get_balanced_sampler(self, generator: Optional[torch.Generator] = None) -> WeightedRandomSampler:
        """返回用于均衡采样的 WeightedRandomSampler"""
        return WeightedRandomSampler(
            weights=self.sample_weights,
            num_samples=len(self.sample_weights),
            replacement=True,  # 允许重复采样（对于少数类）
            generator=generator
        )
