...
class BaseEnv(ABC):
    ...
    # 需要从 <answer> 内容中过滤掉的特殊 token
    # 这些 token 可能被模型错误地生成，会导致对话异常终止
    SPECIAL_TOKENS_TO_FILTER = [
        "<|endoftext|>",  # 模型结束符
        "<|im_end|>",     # Chat template 结束符
        "<|im_start|>",   # Chat template 开始符
        "<|end|>",        # 其他可能的结束符
        "<|pad|>",        # 填充符
    ]
    ...
    def _extract_answer(text: str) -> str:
        """Extract the answer from the text and filter out special tokens."""
        match = re.search(r"<answer>(.*?)</answer>", text, re.DOTALL)
        if match:
            content = match.group(1).strip()
            # 过滤掉特殊 token
            for token in BaseEnv.SPECIAL_TOKENS_TO_FILTER:
                content = content.replace(token, "")
            content = content.strip()
            # 如果过滤后内容为空，返回 None（视为无效）
            if not content:
                return None
            return content
        else:
