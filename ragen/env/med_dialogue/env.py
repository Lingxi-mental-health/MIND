...
class MedicalConsultationEnv(BaseLanguageBasedEnv, gym.Env):
    ...
    def __init__(self, parquet_path: str, env_llm_worker=None, tokenizer=None, max_turns=5):
        ...
        if MedicalConsultationEnv._shared_data is None or MedicalConsultationEnv._parquet_path != parquet_path:
            MedicalConsultationEnv._shared_data, MedicalConsultationEnv._shared_seed_to_index = \
                self._get_data_from_parquet(parquet_path)
            MedicalConsultationEnv._parquet_path = parquet_path

        # ——关键信息：进程内本地引用（以后只用 self._data / self._seed_to_index）——
        self._data = MedicalConsultationEnv._shared_data
        self._seed_to_index = MedicalConsultationEnv._shared_seed_to_index
