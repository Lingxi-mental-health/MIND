    ...
    def _get_data_from_parquet(path: str):
        ...
        for item in df["reward_model"].tolist():
            data.append({
                ...
                "patient_information": item.get("patient_information") if item.get("patient_information") is not None else [],
...
"""
    ...
    def _prepare_question_count_prompt(self, doctor_question):
        """
        准备用于判断问句数量的prompt
        ...
        Returns:
            格式化的prompt用于LLM判断问句数量
        """
        system_prompt = """You are an assistant that counts the number of questions in a given text. 
Please analyze the text and count how many distinct questions are asked.
A question is defined as a sentence that ends with a question mark (?) or asks for information.
Please respond with only a single number (0, 1, 2, etc.) representing the count of questions."""
        ...
        prompt = f"""Text to analyze: {question}

Count the number of questions in the above text. Respond with only a number."""
    ...
    def formulate_output(env_feedback: str, done: bool = False, response: str = None):
        ...
        if response is not None and "<|im_end|>" not in response:
            ...
            output = "\n <|im_start|>user\n" + env_feedback + "<|im_end|>\n"
        if not done:
            output += "<|im_start|>assistant\n"  # 移除<think>，由tokenizer的enable_thinking参数控制
        return output
    ...
    def execute_predictions(cls, envs: List['MedicalConsultationEnvWithPatientLLM'], predictions: List[str], prediction_ids: torch.Tensor, tokenizer: AutoTokenizer):
        ...
        """
        print(f"[EXECUTE_START] Processing {len(envs)} environments")
        print(f"[EXECUTE_START] Predictions: {[p[:50] + '...' for p in predictions]}")
        
        # 添加最少的调试日志
        print(f"[DEBUG] Starting execution for {len(envs)} environments")
        ...
        for i, (env, action, response, response_id, av) in enumerate(zip(envs, cur_actions, predictions, prediction_ids, action_is_valid)):
            ...
            if "Diagnosis" in action:
                ...
                if diagnosis_match:
                    ...
                    # gt_diagnosis = cls._data[env.index]['target']['diagnosis']
                    # 计算诊断的 similarity 分数
                    ...
                    if suggestion_match:
                        ...
                        # gt_suggestion = cls._data[env.index]['target']['recommendation']
        ...
        if llm_envs:
            ...
            # 处理GPU填充
            print(f"[DEBUG] Calling LLM for {len(llm_envs)} environments")
            ...
            for i, (env_idx, response_text) in enumerate(zip(llm_indices, batch_texts)):
                ...
                # 检查是否是重复问题
                if   "Sorry" in response_text or ("asked" in response_text and "before" in response_text)  or "duplicate" in response_text:
                    print(f"Question '{action}' was detected as duplicate")
                    print(f"Response: {response_text}")
                    print(f"History questions: {env.conversation_history}")
                    ...
                    # print(f"Consecutive duplicate count: {env.consecutive_duplicate_count}")

                    if env.consecutive_duplicate_count >= env.max_consecutive_duplicates:
                        # 强制诊断或结束对话
                        obs = "Maximum consecutive duplicates reached. Forcing diagnosis."
                        reward = -5.0
                        env.consecutive_duplicate_count = 0  # 重置重复计数器
                        done=True
            ...
            if len(batch_prompts) == 0:
                print("[WARNING] batch_prompts is empty")
                print(f"[DEBUG] llm_envs count: {len(llm_envs)}")
                print(f"[DEBUG] llm_actions count: {len(llm_actions)}")
                
                finished_count = sum(1 for env in llm_envs if env.finished())
                print(f"[DEBUG] Finished environments: {finished_count}/{len(llm_envs)}")
                
                if finished_count == len(llm_envs):
                    print("[INFO] All environments finished, no more prompts needed")
                else:
                    print("[ERROR] Some environments not finished but no prompts generated")
            ...
            # 处理GPU填充
            print(f"[DEBUG] Calling LLM for patient responses")
            batch_responses = cls._handle_gpu_padding(llm_envs[0].env_llm_worker, batch_data)
            for k, v in batch_responses.batch.items():
                if isinstance(v, torch.Tensor):
                    if v.dtype != torch.int64:
        ...
        print(f"[EXECUTE_END] Returning {len(next_obs)} observations")
        print(f"[EXECUTE_END] Done flags: {dones}")
        return next_obs, dones
