...
class FSDPCheckpointManager(BaseCheckpointManager):
    ...
    def save_checkpoint(self, local_path: str, hdfs_path: str = None, global_step: int = 0, max_ckpt_to_keep=None):
        ...
        if "hf_model" in self.checkpoint_contents:
            ...
            # Collect full model state dict (rank 0 only)
            ...
            if self.rank == 0:
                ...
                print(f"[rank-{self.rank}]: Saving HuggingFace format model to {hf_local_path}")
                
                # Save config and tokenizer
                self.model._fsdp_wrapped_module.config.save_pretrained(hf_local_path)
                self.processing_class.save_pretrained(hf_local_path)
                
                # Save full model weights in HuggingFace format
                try:
                    # Get the original model's name or path for reference
                    config = self.model._fsdp_wrapped_module.config
                    
                    # Use the wrapped module directly and save its state dict
                    unwrapped_model = self.model._fsdp_wrapped_module
                    ...
                    print(f"[rank-{self.rank}]: Successfully saved HuggingFace model weights to {hf_local_path}")
                except Exception as e:
                    print(f"[rank-{self.rank}]: Warning - Failed to save HuggingFace model weights: {e}")
                    print(f"[rank-{self.rank}]: Only config and tokenizer were saved. Use model_merger.py to convert.")
